...
class TestContract(AbstractTestContract):
    """
    run test with python -m unittest contracts.tests.market_factories.test_buy_and_sell_shares
    """
    ...
    def test(self):
        ...
        money_to_earn = self.lmsr.calcEarningsSelling("".zfill(64).decode('hex'), initial_funding, share_distribution,
                                                      outcome, number_of_shares / 2)
        # Approve market contract to transfer shares
        self.event_token(event_hash, outcome, "approve", user, [self.market_factory.address, number_of_shares / 2])
        # Sell shares
        profiling_sell = self.market_factory.sellShares(
            market_hash, outcome, number_of_shares/2, money_to_earn, sender=keys[user], profiling=True)
        ...
        self.assertEqual(self.event_factory.getShares(accounts[user], [event_hash]),
                         [self.b2i(event_hash), 2, number_of_shares/2, 0])
